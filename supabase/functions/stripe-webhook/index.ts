  ...
  try {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // Kept for the Pro flow this project already had.
        if (kind === 'pro' && obj.subscription) {
          await adminClient().from('profiles')
            .update({ stripe_subscription_id: typeof obj.subscription === 'string' ? obj.subscription : obj.subscription?.id })
            .eq('id', userId);
        }
