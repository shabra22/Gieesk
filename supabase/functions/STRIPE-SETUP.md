...
**Where these files must live:** the Supabase CLI's project root is the
folder ABOVE the website — `Downloads\gieeskrecipes-website\supabase\`,
the one with `config.toml`. That is the copy the CLI deploys. The copy
inside the website folder is ignored; keep them in step, or delete the
inner one to avoid confusion.
