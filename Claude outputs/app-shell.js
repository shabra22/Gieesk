...
(function () {
  ...
  // Reusable paywall. The purchase happens in-app through Google Play
  // Billing. It used to open gieesk.com/upgrade.html in a browser, which
  // is exactly what Play's payments policy forbids — an app "may not
  // lead users to a payment method other than Google Play's billing
  // system", and that covers linking to a page that leads to one.
  ...
  function initPaywall() {
    ...
    if (upgradeBtn) {
      upgradeBtn.addEventListener('click', function () {
        // Opens Play's own purchase sheet. Which plan we open with is
        // all this decides; the sheet shows the options.
        if (window.playBilling && window.playBilling.available()) {
          window.playBilling.buy('gieesk_pro_yearly');
          return;
        }
        // Not the app, or the billing plugin isn't installed yet: say so
    ...
    var restoreBtn = document.getElementById('appPaywallRestoreBtn');
    if (restoreBtn) {
      restoreBtn.addEventListener('click', function () {
        if (window.playBilling) window.playBilling.restore();
      });
    }
  }
